...
def _run_policy_output(policy: PolicyCallable, td: TensorDictBase, action_key: str) -> torch.Tensor:
    with torch.no_grad():
        output = policy(td)

    if isinstance(output, TensorDictBase):
        if action_key in output.keys(include_nested=True, leaves_only=True):
            return output.get(action_key)
        if "action" in output.keys(include_nested=True, leaves_only=True):
            return output.get("action")
        raise KeyError(f"TensorDict output from policy is missing '{action_key}' key.")
