...
class CrazyfliePIDController:
    ...
    def get_pid_integrals(self, num_envs: int | None = None) -> torch.Tensor:
        ...
        Returns zeros for PID loops that have not been activated.
        """
        n = num_envs or (self._rate_integral.shape[0] if self._rate_integral is not None else self._num_envs or 1)

        def _get(pid_obj) -> torch.Tensor:
            if pid_obj._integral is not None:
                return pid_obj._integral
            return torch.zeros(n, 3, device=self.device)

        vel_int = _get(self.vel_pid)
        att_int = _get(self.att_pid)
