...
class Drone_cfg:
    ...
    def _build_mixer(self) -> None:
        if float(self.k_eta) <= 0.0:
            raise ValueError("k_eta must be positive for Drone mixing.")
    ...
    def set_physical_params(self, mass: torch.Tensor | float, inertia: torch.Tensor | list) -> None:
        ...
        if self.mass_ref is not None:
            ...
            if float(rel) > 0.3:
                print(
                    f"[WARN] {self.name} mass mismatch: USD={mass_tensor.item():.5f} kg, "
        ...
        if self.inertia_ref is not None:
            ...
            if float(rel.max()) > 0.3:
                print(f"[WARN] {self.name} inertia mismatch: USD={inertia_diag.tolist()}, ref={inertia_ref.tolist()}")
            else:
                print(f"[INFO] {self.name} inertia check: USD={inertia_diag.tolist()}, ref={inertia_ref.tolist()}")


class Propellers:
    def __init__(self, num_envs, drone_cfg: Drone_cfg, dt, use, device="cuda"):
        """Drone motor and aerodynamics model."""
